...
export default function admin(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case ActionTypes.FETCH_STUDENTS.SUCCESS:
      ...
      let students = state.count1 > 1 ? [] : [...state.students, ...payload.students.filter((s) => !student_ids.includes(s.id))];
      return {
        ...state,
        students,
        count1: state.count1 - 1,
    ...
    case ActionTypes.FETCH_TUTORS.SUCCESS:
      ...
      let tutors = state.count2 > 1 ? [] : [...state.tutors, ...payload.tutors.filter((t) => !tutor_ids.includes(t.id))];
      return {
        ...state,
        tutors,
        count2: state.count2 - 1,
