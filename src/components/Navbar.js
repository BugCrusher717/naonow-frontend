...
import { getItemToLocalStorage, setItemToLocalStorage } from "../constants/global";
...
const Navbar = () => {
  ...
  const [language, setLanguage] = useState(parseInt(getItemToLocalStorage('language', 1)));
    ...
    <div className="nav-bar">
      <div className="desktop-version">
        <div className="left-part">
          ...
          <Link to="/dashboard" className="page-title">{t('user_role_dashboard', { user_role: user_role })}</Link>
        </div>
        <div className="right-part">
          <input type="text" placeholder={t('placeholder_search_here')} />
          <Dropdown className="language" icon={language === 1 ? FlagUsa : FlagKorea}
            items={[
              { label: t('korean'), icon: FlagKorea, onClick: onChangeLanguage },
              { label: t('english'), icon: FlagUsa, onClick: onChangeLanguage }
            ]}
          />
          <Dropdown className="settings" icon={IconUser}
            items={[
              { label: t('my_profile'), icon: IconMyprofile, activeIcon: IconMyprofileActive, href: user_role === "tutor" ? '/tutor/profile' : '/student/profile' },
              { label: t('logout'), icon: LogoutImg, onClick: handleLogout }
            ]}
          />
          <Dropdown className="settings" icon={IconNotification}
              ...
              <div key={`notification-${index}`} className="notification">
                <div>
                  <div>
                    ...
                    <span>{t('from_role_name', { role: t(item.role), name: item.name })}</span>
                  </div>
                  <img src={IconNavigate} alt="" onClick={() => onHandle(item)} />
                </div>
              </div>
            }
          />
