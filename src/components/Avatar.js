...
export const Avatar = ({ avatar, name }) => {
  const avatarPresets = [AvatarPreset1, AvatarPreset2, AvatarPreset3, AvatarPreset4, AvatarPreset5, AvatarPreset6, AvatarPreset7, AvatarPreset8, AvatarPreset9];
  return (<>{
    avatar ?
      avatar.indexOf('preset_') > -1 ?
        <img src={avatarPresets[parseInt(avatar.slice(7))]} alt="" className="user-avatar preset" /> :
        <img src={`${process.env.REACT_APP_SERVER_URL}/users/get-avatar?file_name=` + avatar} alt="" className="user-avatar" />
      : <div className="no-avatar"><span>{name}</span></div>
  }</>);
};