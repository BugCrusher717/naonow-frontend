...
export const ModalCancelLesson = ({ visible, reasons, onDismiss, onCancel, lesson }) => {
  ...
  const onSubmit = () => {
    onCancel({
      reasons: checked.map((checked, index) => checked ? index : false).filter(checked => checked),
      id: lesson.id
    });
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex align-items-center justify-content-space-between">
        ...
        <div className="btn"
          onClick={onSubmit}>{t('cancel_lesson')}</div>
      </div>
    </Modal>
  );
}