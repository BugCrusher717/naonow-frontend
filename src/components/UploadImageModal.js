 ...
 **/
  ...
  **/
  const handleUploadImage = (e) => {
    ...
    if (file === null)
      props.uploadImage(`preset_${preset}`, true);
    else
      ...
      <div className="modal-content">
        ...
        <div className="choose-from-presets">
          ...
          <div className="presets"  >
            {[0, 1, 2, 3, 4, 5, 6, 7, 8].map(index => <div key={`presets-${index}`} className={index === preset ? "selected" : ""} onClick={() => setPreset(index)}>
              <Avatar avatar={'preset_' + index} />
            </div>)}
          </div>
        </div>
        <div className="actions">
          <p>{t('ideal_image_size')} <img src={SmileIcon} alt="" /> 200x200px, png/jpeg</p>
          <div className="button-groups">
            <button className="submit-btn" onClick={(e) => handleUploadImage(e)}>{t('confirm')}</button>
            <button className="cancel-btn" onClick={props.hideModal}>{t('cancel')}</button>
          </div>
        </div>
      </div>
    </div>
