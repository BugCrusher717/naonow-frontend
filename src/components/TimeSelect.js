    ...
    >
      <div className="body">
        <div>
          ...
          <DayPicker onDayClick={handleDayClick} modifiers={{ highlighted: selectedDate }} />
        </div>
        <div className="right">
          <div className="start-at">
            <p>{t('time')}</p>
            <div className="times">{times.map((time, index) =>
              <span
                key={index}
          ...
          <div className="lesson-length">
            <p>{t('lesson_length')}</p>
            <div className="durations">{durations.map((duration, index) =>
              <span
                key={index}
                className={duration.value === selectedDuration ? 'active' : ''}
      ...
      <p className="selected-time mobile">{selectedDate.toLocaleDateString() + ' ' + selectedTime}</p>
      <div className="footer">
        <div className="btn outlined" onClick={() => { setSelectedDate(new Date()); setSelectedTime('00:00 AM'); setSelectedDuration(-1); }}>
          {t('clear')}
        </div>
        <p className="selected-time desktop">{selectedDate.toLocaleDateString() + ' ' + selectedTime}</p>
        <div className="btn" onClick={() => {
          let start_at = new Date(`${selectedDate.toLocaleDateString()} ${selectedTime}`);
