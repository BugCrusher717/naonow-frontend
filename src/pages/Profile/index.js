...
const ProfileLayout = ({ user_id }) => {
  ...
  const user = useSelector(state => user_id ? state.admin.user : state.users.user);
  ...
  return (
    <Layout fluid={true}>
      {isTutor !== null && <h4 className="main-title">{isTutor ? t('tutor_profile') : t('student_profile')}</h4>}
