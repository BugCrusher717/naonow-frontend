...
const SERVER_URL = process.env.REACT_APP_SERVER_URL ? process.env.REACT_APP_SERVER_URL : 'https://dev.naonow.contracollective.com'
...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    if ((tutors && tutors.length > 0) || (students && students.length > 0)) {
      ...
      const currentReceiver = receivers && receivers.length > 0 && (!!selected_user_id ? receivers.filter(receiver => receiver.id === selected_user_id)[0] : receivers[0]) || null;
  ...
  const onMessageRecieved = (info) => {
    ...
    if (myMessage) {
      messages[id].push({
        ...
        message,
        image: user.avatar,
        abbrName: `${(user.first_name || '').charAt(0).toUpperCase()}${(user.last_name || '').charAt(0).toUpperCase()}`
      });
    } else {
  ...
  useEffect(() => {
    if (currentReceiver && !loadingPrevMsgs) {
      const isFirst = Object.keys(messages).indexOf(currentReceiver.id.toString()) === -1;
      if (isFirst)
        loadMessage();
      else
        onScrollBottom();
    ...
    <Layout>
      ...
      <div className="view-messages">
        <div className="header-title">
          ...
          <p>{t('talk_with_directly', { person: isTutor ? t('student') : t('tutor') })}</p>
        </div>
        <div className="divider" />
        <div className="messages-window">
          {(!receivers || (receivers && receivers.length === 0)) && <span className="no-chat-history">{t('no_chat_history')}</span>}
          <div className="message-receivers">
            {receivers && receivers.length > 0 &&
              receivers.map((receiver, index) => (
                <div key={`reciever-${index}`} className={`message-receiver ${receiver.id === currentReceiver.id ? "active" : ""}`} onClick={() => selectReceiver(receiver.id)} key={receiver.id}>
                  <div className="message-inner">
                    <Avatar avatar={receiver.image} name={receiver.abbrName} />
                    <p>{receiver.name}</p>
                    {!!receiver.unread && <span className="badge">{receiver.unread}</span>}
                  </div>
                </div>
              ))}
          </div>
          {currentReceiver &&
            <div className="message-content">
              <div className="message-content-header">
                <p>{t('between_you_and_person', { name: currentReceiver.name })}</p>
              </div>
              <div className="messages">
                <div className="load-prev-messages">
                  <a href='#' onClick={loadMessage}>{t('load_messages')}</a>
                </div>
                {
                  messages[currentReceiver.id] && messages[currentReceiver.id].map((message, index) => (
                    <div className={`message-item ${message.myMessage ? "sender-message" : "client-message"}`} key={`message-item-${index}`}>
                      <div className="user-image">
                        <Avatar avatar={message.image} name={message.abbrName} />
                      </div>
              ...
              <div className="message-input submit-action">
                <div className="user-image">
                  <Avatar avatar={user.avatar} name={getAvatarName(user.first_name, user.last_name)} />
                </div>
                <input
                  type="text"
              ...
              <div className='btn-container'>
                <button className="send-message" onClick={sendMessage}>{t('send_message')}</button>
              </div>
              {loadingPrevMsgs &&
