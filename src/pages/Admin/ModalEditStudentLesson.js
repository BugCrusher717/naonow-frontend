...
const ModalEditStudentLesson = ({ student, onDismiss, visible, date, onGoToProfile }) => {
  ...
  const columns_upcoming = [
    {
      ...
      render: (item, record) => (
        <div className="actions">
          <a onClick={() => { setAction('assign'); setLesson(record); }}>{t('assign_substitute')}</a>
          <a onClick={() => { setAction('reschedule'); setLesson(record); }}>{t('reschedule')}</a>
          <a onClick={() => { setAction('delete'); setLesson(record); }} className="outlined">{t('delete')}</a>
        </div>
      )
    }
  ];
  ...
  useEffect(() => {
    if (upcomingLessons) {
      setFilterLessons(upcomingLessons.filter(lesson => isSameDay(currentDate, new Date(lesson.lessonDate))));
    }
        ...
        <div className="scroll-layout">
          {appointment_loading ?
            ...
            <div className="edit-student-lesson-wrapper">
              <UserHeader user={student.user} onAction={onGoToProfile ? onGoToProfile : onDismiss} />
              ...
              <p className="sub-title">{t('scheduled_lessons', { lessons: 3 })}</p>
              <CustomTable className="full-height" data={filterLessons} columns={columns_upcoming} enableSeeAll={false} />
              <Link className="btn-schedule-new-lesson" to={{ pathname: '/admin/schedule-new-lesson', state: { studentId: student.id } }}>{t('schedule_new_lesson')}</Link>
            </div>
          }
        </div>
      </ModalUserInfo>
