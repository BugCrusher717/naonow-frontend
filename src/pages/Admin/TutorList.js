...
const TutorList = () => {
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <div className="with-avatar">
          <Avatar avatar={record.avatar} name={getAvatarName(record.first_name, record.last_name)} />
      ...
      width: 20,
      render: (total_hours) => <span>{total_hours ? total_hours / 60 : 0} {t('hrs')}</span>
    }, {
      ...
      render: (item, record) => (
        <div className="actions">
          <a href="#" onClick={() => setSelectedUser(record)}>{t('view_profile')}</a>
          <a href="#" onClick={() => setDeleteForUser(record)}>{t('delete')}</a>
        </div>
      )
    }
  ];

  const [selectedOption, setSelectedOption] = useState(options.map(opt => ({ ...opt, label: t(opt.label) }))[0]);
  ...
  const fetchData = async () => {
    const res = await dispatch(fetchTutors({
      tutor_name: subname,
      limit,
      offset,
    ...
    <Layout>
      <div className="tutor-list-layout">
        <div className="page-header">
          ...
          <div className="controls">
            <div className="search">
              <input type="text" placeholder={t('search_tutor')} value={subname} onChange={(e) => {
        ...
        <div className="scroll-layout">
          {tutors.length > 0 && <CustomTable data={tutors} columns={columns} enableSeeAll={false} />}
          {loading && <div className="text-align-center">
            <Loader
          ...
          {hasmore && !loading && <div className="see-more-all-btn" onClick={() => {
