  ...
  return (
    ...
    >
      ...
      <div className="feedback-content">
        {tutor_attendance.map((opt, idx) => <Checkbox key={`checkbox-${idx}`} label={tutor_attendance[idx].label} checked={attendance === tutor_attendance[idx].value} onChange={() => setAttendance(tutor_attendance[idx].value)} />)}
      </div>
      ...
      <div className="feedback-content">
        <div className="stars">
          {[0, 1, 2, 3, 4].map((value, index) =>
            <img key={`img-${index}`} src={starIndex >= index ? ImgStarFill : ImgStarOutline} alt="" onClick={() => setStarIndex(index)} />)
          }
        </div>
      </div>
      ...
      <div className="feedback-content">
        {renderFormField('review', t(''), handleChange, formData, formDataError, 'textfield', t('your_review_goes_here'))}
      </div>

      <div className="btn" onClick={onSubmit}>{t('submit')}</div>
    </Modal>
  );
};
