...
import { format, startOfMonth, endOfMonth, startOfWeek, endOfWeek, isSameMonth, isSameDay, addDays, addMonths, subMonths, parseISO, subDays } from 'date-fns'
...
const BookTimeTrial = (props) => {
  ...
  const [monthOption, setMonthOption] = useState(optionMonths[currentDate.getMonth()]);
  ...
  const nextMonth = () => {
    ...
    setYearOption(optionYears.find(year => year.value === newDate.getFullYear()));
  ...
  const onContinue = () => {
    ...
    date = new Date(date.toLocaleDateString() + ' ' + selectedHour.toString().padStart(2, '0') + ':00 ' + (isAM ? 'AM' : 'PM'));
    ...
    <div className="book-trial-lesson-layout">
      ...
      <div className="week">
        ...
        <div className="column col-center">
          <span>{format(startDate, "MMMM dd yyyy")} - {format(endDate, "MMMM dd yyyy")}</span>
        </div>
      ...
      <div className="calendar-wrapper">
        ...
        <div className="choose-by-month">
          <div>
            <Checkbox label="AM" checked={isAM === true} onChange={onChangeAMPM} />
            <span />
            <Checkbox label="PM" checked={isAM !== true} onChange={onChangeAMPM} />
          </div>
          <div>
            <img src={Icon2} alt="" />
            <Select
        ...
        <div className="hours">
          {[0, 1, 2, 3, 4, 5, 6].map((date, dateIndex) => <div key={`date_${dateIndex}`} className="col">
            {hours.map((hour, hourIndex) =>
              <span key={`date_hour_${hourIndex}_${dateIndex}`} className={selectedHour === hour && selectedDateIndex === dateIndex ? "selected" : ""} onClick={() => { setSelectedHour(hour); setSelectedDateIndex(dateIndex); }}><span>{hour}:00</span></span>
            )}</div>)}
        </div>
      </div>
      <div className="btn-continue" onClick={() => onContinue()}>Continue</div>
      <p className="important-note">{t('important_request_trial_teacher_when_purchase_lessons')}</p>
    </div>
  )
}
