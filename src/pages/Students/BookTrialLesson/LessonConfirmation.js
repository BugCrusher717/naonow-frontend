...
const LessonConfirmation = ({ tutor, time, onBack, onContinue }) => {
  ...
  const CancelReasonBox = ({ label, index, checked, onChange }) => {
    return <div className={`cancel-reason ${checked ? 'active' : ''}`}>
      <div>
        <Checkbox checked={checked} onChange={() => onChange(index)} label={label} />
      </div>
    </div>
  }
  return (
    <div className="overview-confirmation">
      ...
      <div className="lesson-detail">
        ...
        <div>
          ...
          <div className="lesson-time-detail">
            <p className="date-time">{format(time, 'MMM dd')}, <strong>{format(time, 'hh:mm')}</strong> {format(time, 'aa')}</p>
      ...
      <div className="btn-confirm" onClick={onContinue}>{t('confirm_lesson')}</div>
    </div>
  )
}
