...
const tutors = [
  {
    ...
    reviews: [
      {
        review: 'Nullam vel neque pulvinar, tristique ipsum sagittis, viverra ligula. Cras eu tortor auctor, vestibulum augue ultricies, efficitur nibh. In ornare maximus leo sit amet lobortis. Fusce quis libero luctus eros convallis finibus laoreet at justo. Nam dui libero, rutrum laoreet sapien nec, lacinia pretium neque. Fusce suscipit libero semper nisl venenatis sodales.',
        points: 4.5,
        avatar: "preset_1",
        from: 'Carolina T.',
...
const SelectTutor = (props) => {
  ...
  useEffect(() => {
    let genderAll = [{ value: 'all', label: t('all') }, ...genders.map(gender => ({ ...gender, label: t(gender.label) }))];
  ...
  const updateFilter = () => {
    ...
    if (genderOption.value !== 'all') {
      filterTutors = filterTutors.filter(tutor => tutor.gender === genderOption.value);
    }
    if (majorOption.value !== 'all') {
      filterTutors = filterTutors.filter(tutor => tutor.major === majorOption.value);
    }
  ...
  return (
    <div className="select-tutor">
      ...
      <div className="scroll-layout">
        <div className="filter">
          <div>
            ...
            <Select
              ...
              getOptionLabel={(option) => option.label}
            />
            <Checkbox label={t('show_favourite_tutors_only')} checked={checkedFavouriteTutors} onChange={onChangeCheckFavouriteTutors} />
          </div>
        ...
        <div className="btn-continue" onClick={onContinue}>{t('continue')}</div>
      </div>
      {modalBriefVisible &&
        <ModalTutorBrief
