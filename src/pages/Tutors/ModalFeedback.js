...
const ModalFeedback = ({ visible, onDismiss, group, user }) => {
  ...
  const [formData, setFormData] = useState({
    feedback: '',
    lesson_topic: '',
    homework_assignment: group?.homework_assignment ? last_part_options.find((o) => o.value === group?.homework_assignment) : null
  })
  ...
  const submitComplete = async () => {
    ...
    try {
      ...
      NotificationManager.error(e.response?.data?.message || t('server_issue'), t)
    }
    setLoading(false);
  }
    ...
    >
        ...
        <div className="student-avatar">
          ...
          <div className="student-basic-info">
            ...
            <div className="student-level">
              <img src={CupImage} width={10} height={10} /> <span>{t('level_n', { n: student?.level || 0 })}</span>
            </div>
      ...
      <div className="lesson-details columns">
        <div>
          ...
          {renderFormField('lesson_topic', '', handleChange, formData, undefined, 'text', 'Lesson Topic Here')}
        </div>
        <div>
          <p className="lesson-details-label">{t('homework_assignment_completed')}</p>
          {renderSelect('homework_assignment', '', 'Choose From List', last_part_options, formData.homework_assignment, handleChange)}
        </div>
        <div>
          <p className="lesson-details-label">{t('feedback')}</p>
          {renderFormField('feedback', '', handleChange, formData, undefined, 'textfield')}
        </div>
      </div>
      <div className="btn" onClick={submitComplete}>{t('submit')}</div>
