...
const LessonFinishModal = ({ visible, onDismiss, group, user }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    last_part_lesson: group?.last_part_lesson ? last_part_options.find((o) => o.value === group?.last_part_lesson) : null
  })
    ...
    try {
      ...
      if (!group.completed) {
        ...
        socket.emit('completeLesson', {
          students: group.students.filter(s => !!s.user?.id).map((s) => s.user.id),
          group: {
            group_id: group.id
          },
          user: {
  ...
  return (
    ...
    >
      ...
      <div className="lesson-details">
        ...
        <div>
          <p className="lesson-details-label">{t('last_part_lesson_completed')}</p>
          {renderSelect('last_part_lesson', '', 'Choose From List', last_part_options, formData.last_part, handleChange)}
        </div>
      </div>
