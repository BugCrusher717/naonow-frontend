    ...
    [
      {
        ...
        dataKey: 'tutor',
        width: 20,
        render: (text, record) => { return <span>{getAbbrName(record.tutor.user.first_name, record.tutor.user.last_name)}</span> }
      }, {
        title: t('level'),
        dataKey: 'student',
        width: 20,
        render: (text, record) => <span>Level #{record.students[0].level ? record.students[0].level : 1}</span>
      }, {
        title: t('level_topic'),
        dataKey: 'lesson',
        width: 20,
    ...
    ] : [
      {
        ...
        dataKey: 'student',
        width: 20,
        render: (text, record) => <span>{t('level_n', { n: record.students[0].level ? record.students[0].level : 1 })}</span>
      }, {
        title: t('level_topic'),
        dataKey: 'lesson',
        width: 20,
      ...
      >
        <div className="scroll-layout">
          <div className="edit-student-lesson-wrapper">
            ...
            <p className="sub-title">{student.first_name}'s {status === 'past' ? t('past_lessons') : t('upcoming_lessons')} {`{${filtered.length}}`}</p>
            {loading ?
              <div className="flex justify-content-center align-items-center">
                <Loader
                  ...
                  width={50}
                /></div> :
              <CustomTable className="full-height" data={filtered} columns={columns} enableSeeAll={false} />
            }
          </div>
        </div>
      </ModalUserInfo>
