...
import { getTutorInfo, updateExceptionDates, updateTutorAvailability, updateTutorInfo } from "../../../actions/tutor";
...
export const AvailabilityView = ({ isAdmin, user_id }) => {
  ...
  const user = useSelector(state => isAdmin ? state.admin.user : state.users.user);
  ...
  const onAddSlot = () => {
    ...
    const fromTime = new Date(`01/01/2021 ${pickedTimes[0].hour}:${pickedTimes[0].min} ${pickedTimes[0].ampm}` || now.get());
    const toTime = new Date(`01/01/2021 ${pickedTimes[1].hour}:${pickedTimes[1].min} ${pickedTimes[1].ampm}` || now.get());
    if (fromTime >= toTime) {
    ...
    for (let slot of _slots) {
      const { from, to } = slot;
      if (index !== editIndex) {
        if ((from <= fromTime && to > fromTime) || (from < toTime && to >= toTime)) {
  ...
  const onAddExceptionDay = () => {
    let from = isTakeDayOff ? undefined : new Date(`${pickedDate.toLocaleDateString()} ${pickedTimes[0].hour}:${pickedTimes[0].min} ${pickedTimes[0].ampm}`);
    let to = isTakeDayOff ? undefined : new Date(`${pickedDate.toLocaleDateString()} ${pickedTimes[1].hour}:${pickedTimes[1].min} ${pickedTimes[1].ampm}`);
    ...
    if (tutorInfo) {
      ...
      Object.keys(tutorInfo).map(key => {
        if (key === 'availabilities') {
          ...
          setExceptions(tutorInfo['exceptiondates'].map(exception => {
            return {
              date: exception.date,
              from: exception.from ? new Date(`${exception.date} ${exception.from}`) : undefined,
              to: exception.to ? new Date(`${exception.date} ${exception.to}`) : undefined
            }
          }));
        } else {
  ...
  const updateProfile = async (reset = false) => {
    ...
    for (let availability of availabilities) {
      ...
      for (let slot of availability.slots) {
        slots.push({
          from: slot.from.getHours().toString().padStart(2, '0') + ':' + slot.from.getMinutes().toString().padStart(2, '0'),
          to: slot.to.getHours().toString().padStart(2, '0') + ':' + slot.to.getMinutes().toString().padStart(2, '0'),
        });
      }
    ...
    for (let exceptiondate of exceptions) {
      ...
      if (exceptiondate.from) {
        exception_.slots.push({ from: format(exceptiondate.from, 'hh:mm aa'), to: format(exceptiondate.to, 'hh:mm aa') })
      }
    ...
    return (
      ...
      >
        ...
        <div className="counters">
          <div>
            <Counter min={0} max={12} onUpdate={(value) => onChangeTime('hour', value, 0)} initValue={pickedTimes[0].hour} />
            <Counter min={0} max={60} onUpdate={(value) => onChangeTime('min', value, 0)} initValue={pickedTimes[0].min} />
          </div>
          <div>
            <Counter min={0} max={12} onUpdate={(value) => onChangeTime('hour', value, 1)} initValue={pickedTimes[1].hour} />
            <Counter min={0} max={60} onUpdate={(value) => onChangeTime('min', value, 1)} initValue={pickedTimes[1].min} />
          </div>
        </div>
        <div className="toggles">
          <Toggle on={{ label: "AM", value: "AM" }} off={{ label: "PM", value: "PM" }} className="timer" onUpdate={(value) => onChangeTime('ampm', value, 0)} initValue={pickedTimes[0].ampm} />
          <Toggle on={{ label: "AM", value: "AM" }} off={{ label: "PM", value: "PM" }} className="timer" onUpdate={(value) => onChangeTime('ampm', value, 1)} initValue={pickedTimes[1].ampm} />
        </div>
        <div className="btn" onClick={onAddSlot}>{t('confirm')}</div>
      </Modal>
    )
  };
        ...
        <div className="inner-content">
          ...
          <div className="select-time">
            ...
            <div className="take-off">
              <Checkbox checked={isTakeDayOff} onChange={() => { setIsTakeOff(!isTakeDayOff) }} label='Take Day Off' />
            </div>
          </div>
        </div>
        <div className="btn" onClick={onAddExceptionDay}>{t('confirm')}</div>
      </Modal>
    )
  };
    ...
    return (
      <div className="slot" key={slot[0]}>
        ...
        <div className="action">
          ...
          <span className="close" onClick={() => onRemoteSlot(t(DAY), index)}></span>
        </div>
      </div>
    )
  };
  ...
  const ExceptionDate = ({ exception, index }) => {
    return (
      <div className="exception-day-slot" key={`exception - day - slot - ${index}`}>
          ...
          {!exception.from ?
            <span>not tutoring</span> : <>
              ...
              <div className="action">
                ...
                <span className="divider" />
                <span className="close" onClick={() => onRemoteExceptionDate(index)}></span>
              </div>
            </>
          }
        </div>
  ...
  return (
    <div className="inner-wrapper">
      ...
      {!isAdmin &&
        <div className="notes">
          <p><strong>{t('note')}</strong> {t('student_time_kst_request_lessons')}</p>
          <br />
          <p><strong>{t('pst_tutors')}</strong> {t('pst_available_tutoring_slots')}</p>
          <p><strong>{t('est_tutors')}</strong> {t('est_available_tutoring_slots')}</p>
          <br />
      ...
      {formData.checked && <>
        ...
        <div className="availabilties-wrapper">
          <div className="header">
            {DAYS.map(day => (<div><span key={`${day}`}>{t(day)}</span></div>))}
          </div>
          <div className="body">
            {DAYS.map(DAY => (
