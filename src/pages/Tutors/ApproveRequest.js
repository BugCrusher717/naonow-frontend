...
import { approveAppointment, cancelAppointment, getAppointments } from '../../actions/appointment';
  ...
  const columns = [
    {
      ...
      render: (item, record) => (
        <div className="actions">
          <a onClick={() => onClickApprove(record)} className="outlined">{t('approve')}</a>
  ...
  return (
    <Layout>
      <div className="main-dashboard">
        ...
        <CustomTable
          data={appointments && appointments.filter(apt => apt.students.length > 0).filter(apt => !apt.students[0].GroupStudent.approved).map(apt => ({
            id: apt.id,
