...
const TutorDashboard = () => {
  ...
  useEffect(() => {
    if (appointments && appointments.list.length > 0) {
      ...
      setUpcomingLessons(appointments.list.filter(apt => today < new Date(apt.start_at)).length);
  ...
  return (
    <div className="main-dashboard">
      ...
      <div className="summary-information">
        <SummaryCard text={t('upcoming_appointments')} icon={CalendarIcon} number={upcomingLessons} />
        <SummaryCard text={t('completed_lessons')} icon={CheckIcon} number={completedLessons} />
        <SummaryCard text={t('students_managed')} icon={ManIcon} number={studentManaged} />
      </div>
      <Lessons appointments={appointments} title={t('upcoming_lessons')} status="upcoming" onAction={onSelectStudent} />
      <Lessons appointments={appointments} title={t('past_lessons')} status="past" onAction={onSelectStudent} onComplete={onCompleteLesson} onFeedback={onFeedback} />
      {visible &&
        <ModalLesson
